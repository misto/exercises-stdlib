...
object Iterables extends FlatSpec with Matchers with org.scalaexercises.definitions.Section {
  ...
  /** `grouped` will return fixed-sized Iterable chunks of an Iterable
    */
  ...
  /** `takeRight` is the opposite of 'take' in Traversable.  It retrieves the last elements of an Iterable.
    */
  def takeRightIterables(res0: List[Int]) {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
    (list takeRight 3) should be(res0)
  }

  /** `dropRight` will drop the number of elements from the right.
    */
  def dropRightIterables(res0: List[Int]) {
    val list = List(3, 5, 9, 11, 15, 19, 21, 24, 32)
    (list dropRight 3) should be(res0)
  }
